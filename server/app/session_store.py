...
Holds the sanitized history a multi-step task needs: what the planner said it would do, and the
last state token. Client action history is read only from each sealed request body, never stored
here. All of it is already sanitized — the server never receives anything else (AGENTS.md
invariant 1) — but three limits apply anyway:
  ...
  **Closed vocabulary.** Action history now lives only in each sealed `PayloadV2.history`, whose
  entries are closed enums (`schemas/payload.py::HistoryEntry`), never free text — so this store
  never has to worry about a page string ending up in the next prompt.
