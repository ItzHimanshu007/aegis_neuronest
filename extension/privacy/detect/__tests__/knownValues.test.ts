...
it('previously tokenized malformed ID remains detected after its label disappears', () => {
  const el = element({ name: 'Reference', labelText: 'Reference', inputType: 'text', value: 'HDFC1001234' });
  const ds = runDetectionCascade({ observation: observation([el]), knownValues: [{ value: 'HDFC1001234', type: 'IFSC' }] }).detections;
...
it('known text occurrences get span lookups for screenshot masks too', () => {
  ...
  const result=runDetectionCascade({observation:o,knownValues:[{value:'HDFC1001234',type:'IFSC'}]});
